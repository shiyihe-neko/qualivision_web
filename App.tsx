...
import { 
  ...
  ChevronLeft, CheckCircle2, Settings2, Sparkles, X
} from 'lucide-react';

const STORAGE_KEY = 'qualivision_multi_projects_v2';
...
const App: React.FC = () => {
  ...
  const [isHistoryOpen, setIsHistoryOpen] = useState(false); // 默认关闭，点击才显示
  ...
  const initialProject: Project = {
    ...
    subtitles: [],
    segments: [],
    timelineCodes: DEFAULT_TIMELINE_CODES,
  ...
  const handleNewProject = () => {
    const newProj = { ...initialProject, id: `proj_${Date.now()}` };
  ...
  const handleFinish = async () => {
    ...
    if (confirm("Finish analysis and save package?")) {
  ...
  return (
    <div className="flex h-screen w-full bg-slate-950 text-slate-100 overflow-hidden font-sans relative">
      ...
      {/* 侧边抽屉：历史记录 (Overlay 模式) */}
      <div 
        className={`fixed inset-y-0 left-0 w-80 bg-slate-900/95 backdrop-blur-xl border-r border-slate-800 z-[100] transition-transform duration-500 shadow-2xl ${isHistoryOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        ...
        <div className="p-4">
          <button 
            ...
            className="w-full flex items-center justify-center gap-2 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-bold transition-all shadow-lg shadow-blue-900/20 mb-4"
          >
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-10 space-y-2 custom-scrollbar">
          {projects.map(p => (
            <div 
              ...
              className={`group p-4 rounded-xl cursor-pointer transition-all border-2 ${activeProjectId === p.id ? 'bg-blue-600/10 border-blue-500/50' : 'bg-slate-800/40 border-transparent hover:border-slate-700 text-slate-400 hover:text-white'}`}
            >
              ...
              <button 
                onClick={(e) => { e.stopPropagation(); setProjects(prev => prev.filter(item => item.id !== p.id)); }}
                className="absolute right-4 top-5 opacity-0 group-hover:opacity-100 p-1 hover:text-red-500 transition-opacity"
              >
                <Trash2 className="w-4 h-4"/>
              </button>
            </div>
          ))}
          {projects.length === 0 && <div className="text-center text-slate-600 text-xs py-20 italic">No historical data.</div>}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {showCodeManager && (
          <CodeManager 
            timelineCodes={project.timelineCodes} transcriptCodes={project.transcriptCodes} notePalette={project.notePalette} 
            onClose={() => setShowCodeManager(false)}
            onUpdateTimelineCodes={(c) => setProject(p => ({...p, timelineCodes: c}))}
        ...
        <header className="h-16 border-b border-slate-800 flex items-center justify-between px-6 bg-[#0a0f1e]/80 backdrop-blur-md shrink-0 z-30">
          <div className="flex items-center gap-6">
            <button 
              ...
              className={`p-2.5 rounded-xl transition-all ${isHistoryOpen ? 'bg-blue-600 text-white shadow-lg' : 'bg-slate-800 text-slate-400 hover:text-white'}`}
          ...
          <div className="flex items-center gap-4">
             ...
             <button 
               onClick={() => setViewMode(viewMode === 'analyze' ? 'visualize' : 'analyze')}
               className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider border transition-all ${viewMode === 'visualize' ? 'bg-blue-600 border-blue-500 text-white shadow-lg' : 'bg-slate-800 border-slate-700 text-slate-300 hover:bg-slate-700'}`}
             >
               <LayoutDashboard className="w-4 h-4" /> {viewMode === 'analyze' ? 'View Report' : 'Editor'}
             </button>

             {viewMode === 'analyze' && (
               <button onClick={handleFinish} className="flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-black uppercase tracking-wider border border-emerald-500/30 transition-all shadow-xl shadow-emerald-900/30">
          ...
          <div className="flex items-center gap-3">
             {/* 终于修复了这里的 Settings 按钮 */}
             <button 
               onClick={() => setShowCodeManager(true)}
               className="flex items-center gap-2 px-4 py-2 bg-slate-800/80 hover:bg-slate-700 text-slate-200 rounded-xl text-xs font-bold border border-slate-700 transition-all"
             >
        ...
        <main className="flex-1 flex flex-col min-h-0 relative">
          {viewMode === 'visualize' ? <Analytics project={project} /> : (
            <div className="flex-1 flex flex-col min-h-0">
              ...
              <footer className="h-64 bg-[#0a0f1e] border-t border-slate-800 shrink-0 shadow-2xl z-20">
                <TimelineCoder 
                  duration={project.duration} currentTime={currentTime} segments={project.segments} codeDefinitions={project.timelineCodes} 
